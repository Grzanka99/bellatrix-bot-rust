...
impl TwitchIrcContext {
    pub fn parse_into(raw_message: String) -> Option<TwitchIrcContext> {
        let mut idx = 0;
        let mut end_idx = 0;
        ...
        if end_idx != raw_message.len() {
            idx = end_idx + 1;
            raw_parameters_component =
                Some(String::from(&raw_message[idx..(raw_message.len() - 1)]));
        }
